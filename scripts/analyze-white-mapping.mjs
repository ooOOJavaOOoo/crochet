...
import { quantizeImage } from '../lib/pattern.ts';
...
function isNearWhitePaletteEntry(entry) {
  const hexToRgb = (hex) => {
  ...
  const hasNameToken = (text, tokens) => tokens.some(t => text.includes(t.toLowerCase()));
...
function isMoonLikeSourcePixel(px) {
  const [r, g, b] = px;
  const avg = (r + g + b) / 3;
  const minChannel = Math.min(r, g, b);
  const maxChannel = Math.max(r, g, b);
...
function isLightCoolSourcePixel(px) {
  const [r, g, b] = px;
  const avg = (r + g + b) / 3;
  const minChannel = Math.min(r, g, b);
...
function isLightPastelNonWhiteSourcePixel(px) {
  const [r, g, b] = px;
  const avg = (r + g + b) / 3;
  const minChannel = Math.min(r, g, b);
  const maxChannel = Math.max(r, g, b);
...
async function analyzeWhiteMappedCells(sourcePixels, gridWidth, gridHeight, paletteIndices, palette, regionName, xStart, xEnd, yStart, yEnd) {
  ...
  let minChannelSum = 0;
  let moonLikeCount = 0, snoutLikeCount = 0, nearWhiteCount = 0;
...
async function main() {
  ...
  try {
    ...
    const snoutResult = await analyzeWhiteMappedCells(
      sourcePixels, GRID_WIDTH, GRID_HEIGHT, paletteIndices, pattern.palette,
      'Snout Strip', 74, 84, 121, 130
    );
    ...
    if (snoutResult) {
      ...
      console.log(`  Moon-like:      ${snoutResult.classification.moonLikeCount} (${((snoutResult.classification.moonLikeCount / snoutResult.cellCount) * 100).toFixed(1)}%)`);
      console.log(`  Snout-like:     ${snoutResult.classification.snoutLikeCount} (${((snoutResult.classification.snoutLikeCount / snoutResult.cellCount) * 100).toFixed(1)}%)`);
      console.log(`  Near-white:     ${snoutResult.classification.nearWhiteCount} (${((snoutResult.classification.nearWhiteCount / snoutResult.cellCount) * 100).toFixed(1)}%)\n`);

      console.log('Sample cells:');
      for (let i = 0; i < Math.min(10, snoutResult.sampleCells.length); i++) {
        const c = snoutResult.sampleCells[i];
        console.log(`  (${c.x.toString().padStart(3)},${c.y.toString().padStart(3)}): RGB(${c.r.toString().padStart(3)},${c.g.toString().padStart(3)},${c.b.toString().padStart(3)}) moon=${c.isMoonLike} snout=${c.isSnoutLike} nearW=${c.isNearWhite}`);
      }
    }
    ...
    if (moonResult) {
      ...
      console.log('Source classification:');
      console.log(`  Moon-like:      ${moonResult.classification.moonLikeCount} (${((moonResult.classification.moonLikeCount / moonResult.cellCount) * 100).toFixed(1)}%)`);
      console.log(`  Snout-like:     ${moonResult.classification.snoutLikeCount} (${((moonResult.classification.snoutLikeCount / moonResult.cellCount) * 100).toFixed(1)}%)`);
      console.log(`  Near-white:     ${moonResult.classification.nearWhiteCount} (${((moonResult.classification.nearWhiteCount / moonResult.cellCount) * 100).toFixed(1)}%)\n`);
